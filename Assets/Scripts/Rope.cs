...
public class Rope : MonoBehaviour
{
    ...
    //(if we need to extend by more than this, create a new
    // rope segment)
    ...
    void CreateRopeSegment()
    {
        ...
        // Get the rigidbidy from the segment
    ...
    // Every frame, increase or decrease the rope's
    // length if necessary
    void Update()
    {
        ...
        if (isDecreasing)
        {
            ...
            // remove the segment; otherwise decrease the top segment's
            // length.
        ...
        if (lineRenderer != null)
        {
            ...
            // number of rope segments, plus a point at
            // the top for the rope anchor, plus a point
            // at the bottom for the gnome
            lineRenderer.positionCount = ropeSegments.Count + 2;

            // Top vertex is always at the rope's location
            lineRenderer.SetPosition(0, this.transform.position);

            // For every rope segment we have, make the corresponding
            // line renderer vertex be at its position
